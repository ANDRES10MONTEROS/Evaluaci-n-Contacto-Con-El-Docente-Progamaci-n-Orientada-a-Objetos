...
func (s *StreamingService) UpdateContentDuration(id string, duration int) error {
	c, err := s.contentRepo.GetContentByID(id)
	if err != nil {
		return err
	}

	return c.SetDuration(duration)
}

func (s *StreamingService) UpdateContentType(id string, ctype models.ContentType) error {
	c, err := s.contentRepo.GetContentByID(id)
	if err != nil {
		return err
	}

	return c.SetType(ctype)
}

func (s *StreamingService) GetContentByID(id string) (*models.Content, error) {
	return s.contentRepo.GetContentByID(id)
}
